...
async def process_metrics(time_taken: float):
    ...
    await valkey.set("average_processing_time", 
                     (await valkey.get("average_processing_time") or 0) + time_taken / (files_processed + 1))

    await valkey.set("last_processing_time", time_taken)

    await valkey.set("latest_file_processed_timestamp", 
