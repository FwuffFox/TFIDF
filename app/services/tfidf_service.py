...
class TFIDFService:
    ...
    async def calculate_tfidf_for_document(self, document_id: str, corpus_id: str = None) -> List[Dict]:
        """
        ...
        """
        logger.info(f"Calculating TF-IDF for document ID: {document_id}, corpus ID: {corpus_id}")
        ...
        try:
            ...
            word_frequencies = await self.document_repository.get_word_frequencies(document_id)
            ...
            if corpus_id:
                ...
                logger.info(f"Using {len(all_documents)} documents from corpus {corpus_id} for IDF calculation")
            else:
                all_documents = await self.document_repository.get_by_user(document.user_id)
                logger.info(f"Using {len(all_documents)} documents from user {document.user_id} for IDF calculation")
            ...
            word_to_doc_count = await self._calculate_document_frequencies(document_ids, {wf.word for wf in word_frequencies})
            
            # Calculate TF-IDF scores
            results = []
            max_frequency = max([wf.frequency for wf in word_frequencies]) if word_frequencies else 1
            ...
            logger.info(f"Successfully calculated TF-IDF for document {document_id}, found {len(results)} terms")
            
            return results
        
        except Exception as e:
            logger.error(f"Error calculating TF-IDF for document {document_id}: {str(e)}", exc_info=True)
            raise
    
    async def _calculate_document_frequencies(self, document_ids: List[str], target_words: Set[str]) -> Dict[str, int]:
        """
        ...
        for doc_id in document_ids:
            try:
                ...
                logger.warning(f"Error processing document {doc_id} for word frequencies: {str(e)}")
                continue
