...
class DocumentRepository:
    ...
    async def add_word_frequencies(self, document_id: str, word_counts: Dict[str, int]) -> None:
        """
    ...
    async def calculate_document_frequency(
        ...
        if collection_id:
            subquery = select(Document.id).join(Document.collections)\
                .where(Collection.id == collection_id)
            query = query.where(WordFrequency.document_id.in_(subquery.scalar_subquery()))
    ...
    async def calculate_total_documents(
        ...
        if collection_id:
            ...
            query = query.join(Document.collections).where(Collection.id == collection_id)
    ...
    async def calculate_tfidf(
        ...
        doc_frequencies = await self.calculate_document_frequency(user_id, collection_id)
